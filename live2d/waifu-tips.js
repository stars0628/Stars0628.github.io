...
! function () {
    ...
    class s {
        ...
        async loadModel(t, s, n) {
            ...
            //以下，直接加载我最喜欢的
            loadlive2d("live2d", `${this.apiPath}get/?id=${5}-${2}`), console.log(`Live2D 模型 ${5}-${2} 加载完成`)
        }
        async loadRandModel() {
            ...
            callback: function () {
                if (select == 0) {
                    ...
                    fetch("https://v1.jinrishici.com/all")
                        .then((e => e.json()))
                        .then((e => {
                            //const t = `这句首诗来自 <span>「${e.origin}」</span>，作者是 <span>${e.author}</span>`;
                            o(e.content, 6e3, 9), setTimeout((() => {
                                //o(t, 4e3, 9)
                            }), 6e3)
                        }))
                }
        ...
        asteroids: {
            ...
            callback: () => {
                if (window.Asteroids) window.ASTEROIDSPLAYERS || (window.ASTEROIDSPLAYERS = []), window.ASTEROIDSPLAYERS.push(new Asteroids);
                    ...
                    e.src = ".\assets\live2d\asteroids.js", document.head.appendChild(e)
                    //本地路径.\assets\live2d\asteroids.js
                }
            }
        },*/
        /*换模型*/
